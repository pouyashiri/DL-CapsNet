...
class CapsuleDropout(keras.Model):
    ...
    def call(self, inputs, training=None):
        ...
        if self.drp_rate != 0:
            ...
            x = K.random_binomial((input_shape[1], input_shape[2]), 1 - self.drp_rate)
            outputs = (x * inputs) / (1 - self.drp_rate)

        return K.in_train_phase(inputs, outputs, training=training)


class CFCLayer(keras.Model):
    def __init__(self, D_in, C_in, D_out, kernel):
        ...
        self.FCs = [layers.Dense(self.D_out, input_dim=self.kernel * self.kernel * self.C_in) for _ in
                    range(self.num_fc_sq ** 2)]

    def compute_output_shape(self, input_shape):
        return (input_shape[0], self.num_fc_sq ** 2, self.D_out)

    # def build(self, input_shape):
    #    self.FCs = [layers.Dense(self.D_out, input_dim=self.kernel * self.kernel * self.C_in) for _ in range(self.num_fc_sq ** 2)]
    def call(self, inputs):
        out_fc = []
        num_ks = self.num_fc_sq
        for ix in range(num_ks):
            for iy in range(num_ks):
                x_part = layers.Lambda(lambda x: x[:, ix:ix + self.kernel, iy:iy + self.kernel, :])(inputs)
                x_part = layers.Flatten()(x_part)
                x_part = self.FCs[ix * num_ks + iy](x_part)  # .unsqueeze(1)
                x_part = K.expand_dims(x_part, axis=1)

                out_fc.append(x_part)
        output = layers.Concatenate(axis=1)(out_fc)
        # print(f'output size = {K.int_shape(output)}')
...
class ConvCapsuleLayer3D(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.W = self.add_weight(
            shape=[self.input_num_atoms, self.kernel_size, self.kernel_size, 1, self.num_capsule * self.num_atoms],
            initializer=self.kernel_initializer,
            name='W')
    ...
    def call(self, input_tensor, training=None):
        ...
        input_tensor_reshaped = K.reshape(input_tensor,
                                          [input_shape[0], 1, self.input_num_capsule * self.input_num_atoms,
                                           self.input_height, self.input_width])

        input_tensor_reshaped.set_shape(
            (None, 1, self.input_num_capsule * self.input_num_atoms, self.input_height, self.input_width))

        # conv = Conv3D(input_tensor_reshaped, self.W, (self.strides, self.strides),
        #                 padding=self.padding, data_format='channels_first')

        conv = K.conv3d(input_tensor_reshaped, self.W, strides=(self.input_num_atoms, self.strides, self.strides),
                        padding=self.padding, data_format='channels_first')

        votes_shape = K.shape(conv)
        _, _, _, conv_height, conv_width = conv.get_shape()
        conv = tf.transpose(conv, [0, 2, 1, 3, 4])
        votes = K.reshape(conv,
                          [input_shape[0], self.input_num_capsule, self.num_capsule, self.num_atoms, votes_shape[3],
                           votes_shape[4]])
        votes.set_shape(
            (None, self.input_num_capsule, self.num_capsule, self.num_atoms, conv_height.value, conv_width.value))

        logit_shape = K.stack(
            [input_shape[0], self.input_num_capsule, self.num_capsule, votes_shape[3], votes_shape[4]])
    ...
    def compute_output_shape(self, input_shape):
        ...
        for i in range(len(space)):
            new_dim = conv_output_length(space[i], self.kernel_size, padding=self.padding, stride=self.strides,
                                         dilation=1)
        ...
        if self.r_num == 1:
            ...
            def rt(ub):
                ...
                for r in range(self.r_num):
                    if r > 0:
                        c = K.expand_dims(K.softmax(b * self.b_alphas[
                            r])) * self.ch_j  # distribution of weighs of capsules in I across capsules in J
                        c = K.stop_gradient(c)
                    else:
                        c = 1.0
...
class AffTrans(layers.Layer):

    def __init__(self, num_capsule, dim_capsule, channels, routings=3,
                 kernel_initializer='glorot_uniform',
                 **kwargs):
        super(AffTrans, self).__init__(**kwargs)
        self.num_capsule = num_capsule
        self.dim_capsule = dim_capsule
        self.routings = routings
        self.channels = channels
    ...
    def call(self, inputs, training=None):
        ...
        # inputs_hat.shape = [None, num_capsule, input_num_capsule, dim_capsule]
        inputs_hat = K.map_fn(lambda x: own_batch_dot(x, W2, [2, 3]), elems=inputs_tiled)
        # print(f'???????????????????????{K.int_shape(inputs_hat)}')
        # exit(1)
        return inputs_hat

    def compute_output_shape(self, input_shape):
        return tuple([None, self.num_capsule, self.input_num_capsule, self.dim_capsule])


class DRouting(layers.Layer):

    def __init__(self, num_capsule, dim_capsule, channels, routings, kernel_initializer='glorot_uniform',
                 **kwargs):
        super(DRouting, self).__init__(**kwargs)
        self.num_capsule = num_capsule
        self.dim_capsule = dim_capsule
        self.routings = routings
        self.channels = channels
    ...
    def call(self, inputs, training=None):
        ...
        for i in range(self.routings):
            ...
            # then matmal: [input_num_capsule] x [input_num_capsule, dim_capsule] -> [dim_capsule].
            # outputs.shape=[None, num_capsule, dim_capsule]
            outputs = squash(own_batch_dot(c, inputs, [2, 2]) + self.B)  # [None, 10, 16]
            ...
            if i < self.routings - 1:
                ...
                # then matmal: [dim_capsule] x [input_num_capsule, dim_capsule]^T -> [input_num_capsule].
                # b.shape=[batch_size, num_capsule, input_num_capsule]
                b += own_batch_dot(outputs, inputs, [2, 3])
        # End: Routing algorithm -----------------------------------------------------------------------#
